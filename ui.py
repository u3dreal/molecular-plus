    ...
    def draw(self, context):
        ...
        if obj and obj.particle_systems.active:
            ...
            if not scn.mol_simrun and not psys.point_cache.is_baked:
                ...
                if not psys.point_cache.info.startswith(
                    "1 frames"
                ) or psys.point_cache.info.startswith("0 frames"):
                    row = box.row()
                    row.operator(
                        "object.resume_sim", icon="TRACKING_FORWARDS", text="Resume"
                    )
                    row = box.row()
                    row.operator(
                        "object.bake_sim", icon="REC", text="Current Cache to Bake"
                    )

            if psys.point_cache.is_baked and not scn.mol_simrun:
                row.operator(
                    "object.clear_pcache", icon="CANCEL", text="Free All Bakes"
                )
...
class MS_PT_MolecularCreatePanel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        if (
            bpy.app.version[0] >= 4
            or bpy.app.version[0] == 4
            and bpy.app.version[1] >= 1
        ):
            row.operator(
                "molecular_operators.molecular_makegrid2d",
        ...
        row = layout.row()
        row.operator(
            "molecular_operators.molecular_makegrid3d",
            icon="MOD_REMESH",
            text="3D Grid",
        )
        row.operator(
            "molecular_operators.molecular_makecollider",
            icon="MOD_PHYSICS",
            text="Collider",
        )
        row = layout.row()
        row.operator(
            "molecular_operators.molecular_maketape",
            icon="MOD_SIMPLIFY",
            text="Sticky Tape",
        )
        row.operator(
            "molecular_operators.molecular_makepin",
            icon="MOD_SIMPLIFY",
            text="Sticky Pin",
        )
...
class MS_PT_MolecularUVToolsPanel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        row = layout.row()
        row.operator(
            "object.mol_cache_global_uv", text="Cache Object UVs", icon="GEOMETRY_NODES"
        )
        row = layout.row()
        row.operator(
            "object.mol_cache_active_uv", text="Cache active UVs", icon="GEOMETRY_NODES"
        )
...
class MS_PT_MolecularToolsPanel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        row = layout.row()
        row.operator(
            "object.convert_to_geo", text="Convert to Geonodes", icon="GEOMETRY_NODES"
        )
        row = layout.row()
...
class MS_PT_MolecularDonorPanel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        ).url = "https://www.paypal.com/cgi-bin/webscr?cmd=_s-xclick&hosted_button_id=J7W7MNCKVBYAA"
        row.operator(
            "wm.url_open", text=" Discord ", icon="URL"
        ).url = "https://discord.com/invite/tAwvNEAfA3"
        row.operator(
            "wm.url_open", text=" q3de ", icon="URL"
        ).url = "http://www.q3de.com/research/molecular"
...
class MS_PT_MolecularPanel(bpy.types.Panel):
    ...
    def poll(cls, context):
        if (
            context.object.particle_systems.active
            and context.object.particle_systems.active.settings.mol_active == True
        ):
            return True
    ...
    def draw(self, context):
        ...
        if psys.settings.mol_density_active:
            ...
            if psys.settings.mol_matter != "-1":
                pmass = (psys.settings.particle_size**3) * float(
                    psys.settings.mol_matter
                )
                density = float(psys.settings.mol_matter)
                row.label(
                    icon="INFO",
                    text="Kg per CubeMeter:" + str(round(density, 5)) + " kg",
                )
            else:
            ...
            row = subbox.row()
            row.label(
                icon="INFO", text="Mass per Particle: " + str(round(pmass, 5)) + " kg"
            )
            row = subbox.row()
            row.label(
                icon="INFO",
                text="Total system approx weight: "
                + str(round(len(psys_eval.particles) * pmass, 4))
                + " kg",
            )
        ...
        row.prop(
            ...
            icon="PHYSICS",
            text="Collision with Others",
        )
        if psys.settings.mol_othercollision_active:
            box.prop(psys.settings, "mol_collision_group", text="Collision Group:")
        if (
            psys.settings.mol_selfcollision_active
            or psys.settings.mol_othercollision_active
        ):
            row = box.row()
        ...
        row = box.row()
        row.prop(
            psys.settings, "mol_links_active", icon="CONSTRAINT", text="Link at Birth"
        )
        row.prop(
            psys.settings,
            "mol_other_link_active",
            icon="CONSTRAINT",
            text="Link with Others at Birth",
        )
...
def append_to_PHYSICS_PT_add_panel(self, context):
    ...
    if psys.settings.mol_active:
        col.operator("molecular_operators.molecular_remove", text="Molecular", icon="X")
    else:
        col.operator(
            "molecular_operators.molecular_add", text="Molecular", icon="MOD_PARTICLES"
        )


panel_classes = (
    MS_PT_MolecularPanel,
    MolecularAdd,
    MolecularRemove,
    MS_PT_MolecularHelperPanel,
    MS_PT_MolecularCreatePanel,
    MS_PT_MolecularInspectPanel,
    MS_PT_MolecularUVToolsPanel,
    MS_PT_MolecularToolsPanel,
    MS_PT_MolecularDonorPanel,
)
