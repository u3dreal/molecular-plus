...
process = Popen(
    [sys.executable, "setup_arm64.py", "bdist_wheel"], stdout=PIPE, stderr=PIPE
)
...
for root, _, files in walk("dist"):
    for file in files:
        if file.endswith(".whl"):
            ...
            wheel_found = True
            print(f"Moved wheel to {destination}", file=sys.stderr)

if not wheel_found:
    print("No wheel file found in dist/!", file=sys.stderr)
    sys.exit(1)
...
with ZipFile(
    "molecular-plus_{}_{}_{}_arm64.zip".format(version, v, name),
    "w",
